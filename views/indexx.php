...
<?php
	if(\Yii::$app->user->can('moderator')){
		...
		echo Html::a($organization->title,  ['index', 'id_obl'=>$organization->id], ['class' => 'btn btn-primary']);
		}
		}
	}
?>
                ...
                foreach ($model as $key => $mod) {
                    if($mod->id_type==$typ->id){ 
                        echo '<tr>
                                ...
                                <td class="ent_k_d"'.($mod->d_enter_control_p<=time() ? ($mod->d_enter_controll==0 ? 'bgcolor="#B22222"':'' ): ($mod->d_enter_control_p<=$mod->d_enter_controll ? 'bgcolor="#FFD700"':' ')).' data-key="'. $mod->id.'" data-type="d_enter_control">'.($mod->d_enter_controll!=0 ? date('d.m.Y', $mod->d_enter_controll) : 'Невказано').'</td>
                                ...
                                <td class="ent_k_d" '.($mod->d_shadow_work_p<=time() ? ($mod->d_shadow_work==0 ? 'bgcolor="#B22222"':'' ): ($mod->d_shadow_work_p<=$mod->d_shadow_work ? 'bgcolor="#FFD700"':' ')).'data-key="'. $mod->id.'" data-type="d_shadow_work">'.($mod->d_shadow_work!=0 ? date('d.m.Y', $mod->d_shadow_work) : 'Невказано').'</td>
                                ...
                                <td class="ent_k_d" '.($mod->d_end_interim_work_p<=time() ? ($mod->d_end_interim_work==0 ? 'bgcolor="#B22222"':'' ): (d_end_interim_work_p<=$mod->d_end_interim_work ? 'bgcolor="#FFD700"':' ')).'data-key="'. $mod->id.'" data-type="d_end_interim_work">'.($mod->d_end_interim_work!=0 ? date('d.m.Y', $mod->d_end_interim_work) : 'Невказано').'</td>
                                ...
                                <td class="ent_k_d"  '.($mod->d_end_work_p<=time() ? ($mod->d_end_work==0 ? 'bgcolor="#B22222"':'' ): ($mod->d_end_work_p<=$mod->d_end_work ? 'bgcolor="#FFD700"':' ')).'data-key="'. $mod->id.'" data-type="d_end_work">'.($mod->d_end_work!=0 ? date('d.m.Y', $mod->d_end_work) : 'Невказано').'</td>
                                ...
                                <td class="ent_k_d" '.($mod->d_assept_work_p<=time() ? ($mod->d_assept_work==0 ? 'bgcolor="#B22222"':'' ): ($mod->d_assept_work_p<=$mod->d_assept_work ? 'bgcolor="#FFD700"':' ')).'data-key="'. $mod->id.'" data-type="d_assept_work">'.($mod->d_assept_work!=0 ? date('d.m.Y', $mod->d_assept_work) : 'Невказано').'</td>
