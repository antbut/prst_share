...
class MainSearch extends Main
{
    ...
    {
        return [
            [['id', 'pidr', 'status_pidr', 'status_objekt', 'id_obl', 'date_create', 'id_project_type'], 'integer'],
    ...
    public function searchorg($params)  // сортировка для подрядчика
    {
     //   $query = Main::find()->where(['pidr'=>User::findOne(Yii::$app->user->id)->id_organization]);
       ...
       //  $query = Main::find()->where(['OR',['pidr'=>User::findOne(Yii::$app->user->id)->id_organization], ['status_pidr'=>2 ]]);
        ...
        $query = Main::find()->where(['AND',['pidr'=>User::findOne(Yii::$app->user->id)->id_organization], ['>','status_objekt',0]]);
	...
	public function searcbasket($params)  // сортировка для орзины
    {
     //   $query = Main::find()->where(['pidr'=>User::findOne(Yii::$app->user->id)->id_organization]);

       //  $query = Main::find()->where(['OR',['pidr'=>User::findOne(Yii::$app->user->id)->id_organization], ['status_pidr'=>2 ]]);

        $query = Main::find()->where(['AND', ['id_obl'=>ParentOrg::GetOblPidr(User::findOne(Yii::$app->user->id)->id_organization)], ['OR',['status_pidr'=>2], ['status_pidr'=>4]]]);
    ...
    public function searcbasket_obl($params)  // сортировка для орзины
    {
     //   $query = Main::find()->where(['pidr'=>User::findOne(Yii::$app->user->id)->id_organization]);

       //  $query = Main::find()->where(['OR',['pidr'=>User::findOne(Yii::$app->user->id)->id_organization], ['status_pidr'=>2 ]]);

        $query = Main::find()->where(['AND', ['id_obl'=>User::findOne(Yii::$app->user->id)->id_organization], ['OR',['status_pidr'=>2], ['status_pidr'=>4]]]);
	...
	public function searcbasketadmin($params)  // сортировка для орзины
    {
     //   $query = Main::find()->where(['pidr'=>User::findOne(Yii::$app->user->id)->id_organization]);
